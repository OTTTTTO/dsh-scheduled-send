...
const mkdtemp = async () => await fs.mkdtemp('/tmp/dss-sched-');
...
test('restore after restart: overdue item is re-delivered (补发) on tick', async () => {
  const dir = await mkdtemp();
  const clock = fakeClock();
  const delivered = [];
  const s1 = await createScheduler({ dataDir: dir, clock, timers: manualTimers(), deliver: async (it) => delivered.push(it) });
  await s1.schedule({ content: '重启后补发', sendAt: 10_000, conversationId: 'sess-1' });
  await s1.dispose();
  ...
  assert.equal(delivered[0].content, '重启后补发');
  assert.equal((await fs.readFile(`${dir}/tasks.json`, 'utf8')).match(/重启后补发/) ? true : false, false, 'delivered item removed from disk');
  await s2.dispose();
});
...
test('schedule validates content and sendAt', async () => {
  const dir = await mkdtemp();
  const sched = await createScheduler({ dataDir: dir, clock: fakeClock(), timers: manualTimers(), deliver: async () => {} });
  await assert.rejects(() => sched.schedule({ content: '  ', sendAt: 5 }), /内容/);
